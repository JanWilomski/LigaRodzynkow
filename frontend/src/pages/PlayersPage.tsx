...
import { getPlayers, createPlayer, deletePlayer, getStandings } from '@/lib/api'
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--color-muted)] mt-0.5">
            {players?.length ?? 0} {players?.length === 1 ? 'zawodnik' : 'zawodników'} w lidze
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {players.map((player) => {
            ...
            const hasSets = stats && stats.setsPlayed > 0
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  ...
                  <Button
                    ...
                    disabled={hasSets || deleteMutation.isPending}
                    title={hasSets ? 'Nie można usunąć - zawodnik ma rozegrane sety' : 'Usuń'}
                ...
                {stats && stats.setsPlayed > 0 && (
                  <div className="grid grid-cols-3 gap-2 pt-3 border-t border-[var(--color-border)]">
                    <Stat label="Sety" value={stats.setsPlayed} />
                    <Stat label="W—P" value={`${stats.setsWon}—${stats.setsLost}`} />
                    <Stat
                      label="Winrate"
