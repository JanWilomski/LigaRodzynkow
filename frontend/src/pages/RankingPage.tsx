...
import { getStandings, getSets } from '@/lib/api'
...
export function RankingPage() {
  ...
  const mostActive = standings?.slice().sort((a, b) => b.setsPlayed - a.setsPlayed)[0]
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="text-sm text-[var(--color-muted)] tabular">
                  {formatPercent(leader.winrate)} · {leader.setsWon}W
                </div>
              </div>
            </div>
          )}
          ...
          {mostActive && (
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="text-sm text-[var(--color-muted)] tabular">
                  {mostActive.setsPlayed} rozegranych
                </div>
              </div>
            </div>
          )}
          ...
          <table className="w-full">
            <thead>
              <tr className="text-[10px] uppercase tracking-wider text-[var(--color-subtle)] border-b border-[var(--color-border)]">
                ...
                <th className="text-right font-medium px-3 py-3 w-20">Sety</th>
  ...
  return (
    <tr className="border-b border-[var(--color-border)] last:border-b-0 hover:bg-[var(--color-surface-elevated)] transition-colors">
      ...
      <td className="px-3 py-4 text-right text-sm tabular">
        <span className="text-[var(--color-success)] font-medium">{standing.setsWon}</span>
        <span className="text-[var(--color-subtle)] mx-1">—</span>
        <span className="text-[var(--color-muted)]">{standing.setsLost}</span>
      </td>
      <td className="px-6 py-4">
...
function useStandings() {
  const { data } = useQuery({ queryKey: ['standings'], queryFn: getStandings })
  return data
}
